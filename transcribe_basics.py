...
def upload_bucket(bucket_name ,local_file_path, obj_key):
    ...
    print('-'*88)
    object_key =obj_key
    # bucket_name = f'my-bucket-{time.time_ns()}'
    # bucket_name ='myfilestorage1'
    print(f"Creating bucket {bucket_name}.")
    # local_file_path ='demo.wav'
    # object_key ='demo.wav'
   ...
   # media_file_name = 'demo.wav'
    #media_object_key = 'demo.wav'
    print(f"Uploading media file {local_file_path}.")
...
def Transcribe(local_file_path ,object_key):
   ...
   # logging.basicConfig(level=logging.INFO, format='%(levelname)s: %(message)s')
    ...
    create_vocabulary(
        vocabulary_name, 'ar-AE', transcribe_client,
        phrases=[
    ...
    start_job(
        job_name_vocabulary_list, media_uri, 'wav', 'ar-AE', transcribe_client,
        vocabulary_name)
